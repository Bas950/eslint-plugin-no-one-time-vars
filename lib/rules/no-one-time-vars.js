  ...
  create(context) {
    ...
    function collectOneTimeVariables(scope) {
      ...
      for (const variable of wVariables) {
        ...
        if (node.init) {
          ...
          switch (node.init.type) {
            case "ArrayExpression": {
              if (node.init.elements.length >= 4) return;
              break;
            }
            ...
            case "ObjectExpression": {
              if (
                ...
                return;
              if (node.init.properties.length >= 5) return;
              break;
            }

            default: {
              if (code.getText(node.init).length >= 100) return;
              break;
            }
          }
        }
        ...
        switch (node.id.type) {
          case "ArrayPattern": {
            if (node.id.elements.filter(Boolean).length > 1) return;
            break;
          }

          case "ObjectPattern": {
            if (node.id.properties.length > 1) return;
            break;
          }
        }

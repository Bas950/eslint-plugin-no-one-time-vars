...
module.exports = {
    ...
    schema: [
      {
        ...
        properties: {
          ...
          ignoreTemplateLiterals: {
            type: "boolean",
            description: "Ignores template literals",
            default: false
          },
          allowInsideCallback: {
            type: "boolean",
  ...
  create(context) {
      ...
      for (const variable of wVariables) {
        ...
        if (node.init) {
          ...
          switch (node.init.type) {
            ...
            default: {
              if (
                code.getText(node.init).length >= options.allowedVariableLength
              )
                continue;
              break;
            }
        ...
        if (
          ...
          [node.parent.parent.type, identifier.parent.type].some((type) =>
    ...
    return {
      ...
      "Program:exit"() {
        collectOneTimeVariables(code.getScope(code));
        ...
        for (const value of Object.values(variables)) {
          ...
          if (
            ...
            value.node.parent.type === "TemplateLiteral"
          )
            continue;
          ...
          context.report({
            ...
            *fix(fixer) {
              if (value.node.parent.parent.specifiers?.length > 1) return;
              ...
              } else yield fixer.removeRange(value.varNode.parent.range);
              ...
              if (value.node.parent.type === "Property") {
              ...
              } else if (value.node.parent.type === "ExportSpecifier") {
                const declarationText = code.getText(value.varNode.parent);
                yield fixer.replaceTextRange(
                  value.node.parent.parent.range,
                ...
                let outout = code.getText(value.init);

                if (
                  ["LogicalExpression", "TSAsExpression"].includes(
                    value.init.type
                  )
                )
                  outout = `(${outout})`;

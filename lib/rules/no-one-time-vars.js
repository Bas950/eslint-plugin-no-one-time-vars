...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * Get the parent node of the given node that has the given type.
     * @param {import('eslint').Rule.Node} node
    ...
    function getBlockStatementStartLine(node) {
      return getFirstParent(node, "BlockStatement")?.parent?.loc?.start?.line;
    }
    ...
    return {
      Identifier(node) {
        if (isVariableDeclarator(node) || isArrayOrObjectPattern(node)) {
          ...
          if (node.parent.object?.callee?.name === "require") return;

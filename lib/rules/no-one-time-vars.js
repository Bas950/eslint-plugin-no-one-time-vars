...
module.exports = {
  ...
  create(context) {
    ...
    function addVariable(node, params = {}) {
      ...
      if (hasKey(node.name) && !params.noCheck) {
        const scope = variables[node.name].scope;

        if (scope && scope !== params.scope) {
          return addVariable(node, {
            name: `${node.name}-${getLastBlockStatementStartLine(node)}`,
            noCheck: true
          });
    ...
    function increaseUsageCount(node, key) {
      if (!key) if (getKey(node)) return increaseUsageCount(node, getKey(node));

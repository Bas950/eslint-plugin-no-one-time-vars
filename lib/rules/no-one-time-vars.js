...
module.exports = {
  ...
  create(context) {
    ...
    /**
     * Get the start line of the given node's first last statement.
     * @param {import('eslint').Rule.Node} node
     * @returns The start line of the node's first last statement.
     */
     ...
     * @param {import('eslint').Rule.Node} node
     * @param {string} name
     * @returns the first variable declaration node that has the given name.
     */
    ...
    return {
      Identifier(node) {
        if (isVariableDeclarator(node) || isArrayOrObjectPattern(node)) {
          ...
          variables[
            `${node.name}-${getBlockStatementStartLine(

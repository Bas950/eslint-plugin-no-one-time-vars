...
module.exports = {
  ...
  create(context) {
    ...
    function getInit(node) {
      if (node?.init) return node?.init;
      let init = node?.init;
    ...
    function getBlockStatementStartLine(node) {
      return getFirstParent(node, "BlockStatement")?.loc?.start?.line;
    }
     ...
     * @param {{node: import("eslint").Rule.Node, callee: boolean}} data
     * @returns
     */
    ...
    return {
      Identifier(node) {
        if (isVariableDeclarator(node) || isArrayOrObjectPattern(node)) {
          ...
          if (varNode?.init) {
            switch (varNode.init.type) {
              case "ArrayExpression":
                if (varNode.init.elements?.length >= 6) 
                  return;

              case "ObjectExpression":
                if (varNode.init.properties?.some((x) => sourceCode.getText(x)?.length >= 50)) 
                  return;

              default:
                if (sourceCode.getText(varNode.init).length >= 130)
                  return;
            }
          }

          if (variables[node.name]) {
            // If the variable is already in the list, we create a new one with a different name.
            if (statement) {
              // If the variable is declared inside a block statement, we add dash and the line number to the name.
              variables[`${node.name}-${statement}`] = {
                init: getInit(node.parent),
                varNode: varNode,
                name: node.name,
                node: node,
                count: 0
            ...
            // If the variable is not in the list, we add it.
            variables[node.name] = {
              varNode: varNode,
              init: getInit(node.parent),
              name: node.name,
          ...
          if (
            ["ForOfStatement", "ForInStatement", "ForStatement"].includes(
              node.parent.parent.type
            )
          )
            return; // If the variable is used inside a for-of loop, we don't count it.
        ...
        for (const value of Object.values(variables)) {
          ...
          if (value.count === 1) {
            context.report({
              ...
              fix: (() => {
                if (value.varNode?.type === "ObjectPattern") return;
                if (value.varNode?.id?.type === "ArrayPattern") return;
                ...
                return function* (fixer) {
                  ...
                  if (value.node.parent.type === "Property") {
                    if (value.node.parent.shorthand) {
                      // If the property is a shorthand, we make it a normal property use the init value (of the variable) as value of the property.
                    ...
                    // Replace the variable with the init value (of the variable).
                    yield fixer.replaceTextRange(
                      value.node.range,
                      sourceCode.getText(value.init)
                    );
